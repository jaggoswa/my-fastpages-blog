...
class ClfCallback(Callback):
    def __init__(self, learner, bs, X_train, y_train, X_test, y_test):
        super().__init__(learner)
        self.bs = bs
        self.losses = []
        self.batch_losses = []
    ...
    def epoch_end(self):
        for layer, name, fnval, grval in self.learner.model.params_and_grads():
            ...
            self.gradhist[layer.name+'_'+name].append(grval)
        eloss = take_mean(self.batch_losses[-self.bpe:], self.bpe, self.afrac)
        probs_train = self.learner.model(self.X_train)
        probs_test = self.learner.model(self.X_test)
        pred_train = 1*(probs_train >= 0.5)
        pred_test = 1*(probs_test >= 0.5)
        accuracy_train = np.mean(self.y_train == pred_train)
        accuracy_test = np.mean(self.y_test == pred_test)
        self.losses.append(eloss)
